...
bool AllocatePGMImage(PgmImage &image, int max_val, int w, int h) {
  ...
  std::cout << "MAX VAL IN ALLOCATE: " << max_val << std::endl;
  if (max_val > 255) {
...
bool ReadPGMImage(PgmImage &image, std::string filename) {
  ...
  // Read Image header
  ReadPGMImageHeader(type, width, height, max_val, fin);
...
bool WritePGMImage(PgmImage image, std::string filename) {
  ...
  std::cout << "MAX VAL IN WRITE: " << image.max_val << std::endl;
